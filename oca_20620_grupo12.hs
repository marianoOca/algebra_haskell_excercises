...
generarClaves p q | n <= 127 = ((0,0),(0,0))                                    -- esto indica que los primos son muy chicos
          ...
          where n = p*q
                m = (p-1)*(q-1)
                e = encontrarGranE (m-2) (m-3)  -- encontrarMiniE (m-2) 2        --si se quiere un e más chico
                d = encontrarD e m 1
...
encontrarA b n e a | mod (b^e - a) n == 0 = a                                           -- es igual a n | b^e-a
                   | otherwise = encontrarA b n e (a+1)
...
--4a--
sirveE :: (Integer, Integer) -> [Integer] -> Bool
sirveE _ [] = True
...
tirameUnaE d m e | mod (e*d - 1) m == 0 = e
                 | otherwise = tirameUnaE d m (e+1)
